...
-- DDL CREATE TABLE과
-- DML INSERT SELECT UPDATE DELETE 전반적으로
-- 한번 봤다.
-- 옵션들을 추가해서 명령들을 학습
...
-- 현재 테이블 구조상 추가할 때 
-- 학번, 이름, 전화번호, 나이, 주소순으로 추가를 해야한다.
-- 그런데 원고에는 학번, 이름, 나이, 주소, 전화번호순으로 작성 되어있다. 
-- 그러다보니 자꾸 입력 실수를 하게 된다.
-- 그래서 INSERT 문에 옵션을 사용해서 순서를 바꾸고자 한다.
INSERT INTO tbl_student(st_num, st_name, st_age, st_addr, st_tel)
VALUES('003', '홍길동', 12, '서울시','010-333-3333');

-- INSERT 실행할 때 TABLE(칼럼 리스트) 형식으로 작성을 하면
...
UPDATE tbl_score SET st_kor = 90, st_eng = 80, st_math = 70 WHERE st_num = '001';
UPDATE tbl_score SET st_kor = 91, st_eng = 86, st_math = 72 WHERE st_num = '002';
UPDATE tbl_score SET st_kor = 92, st_eng = 85, st_math = 73 WHERE st_num = '003';
UPDATE tbl_score SET st_kor = 93, st_eng = 84, st_math = 74 WHERE st_num = '004';
UPDATE tbl_score SET st_kor = 94, st_eng = 83, st_math = 75 WHERE st_num = '005';
UPDATE tbl_score SET st_kor = 95, st_eng = 82, st_math = 76 WHERE st_num = '006';
UPDATE tbl_score SET st_kor = 96, st_eng = 81, st_math = 77 WHERE st_num = '007';

SELECT * FROM tbl_score;

SELECT st_kor, st_eng, st_math, st_kor + st_eng + st_math AS 총점 FROM tbl_score;

-- 각 칼럼의 합계와 평균을 계산해서 별도 칼럼으로 보여주는 방법
SELECT st_kor AS 국어점수, st_eng AS 영어점수, st_math AS 수학점수, st_kor + st_eng + st_math AS 총점,
(st_kor + st_eng + st_math) / 3 AS 평균 FROM tbl_score;
...
-- 예) ROUND(3.232323,2) => 3.23
--     ROUND(3.236234,2) => 3.24
SELECT st_kor AS 국어점수, st_eng AS 영어점수, st_math AS 수학점수, st_kor + st_eng + st_math AS 총점,
ROUND((st_kor + st_eng + st_math) / 3,2) AS 평균 FROM tbl_score;
...
-- DUAL TABLE
-- 간단 계산식을 계산해보거나
-- 내장 함수등을 테스트 해 보고자 할 때
-- 사용하는 DUMMY TABLE
-- ORACLE 문법 구조상 SELECT 다음에 FROM이 없으면 
-- 오류가 발생하므로 임시로 사용할 수 있는 DUMMY TABLE을 
-- 만들어두었다.
SELECT * FROM DUAL;
