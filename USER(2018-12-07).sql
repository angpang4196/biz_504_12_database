...
-- 한 사람의 데이터로 중복된 것이 확인되었다.
-- 다시 원본 문서를 봤더니 두번째 홍길동의
-- 주소가 서울이 아닌 부산인 것으로 확인되었다.
-- 그래서 두번째 홍길동의 주소를 부산으로 수정하려고한다.
-- 다시 한번 SELECT 해서 홍길동의 데이터를 확인하자.
SELECT * FROM tbl_addr WHERE st_name = '홍길동'; -- st_name칼럼의 값이 홍길동인 경우만 보여라.

-- 데이터의 변경 실행
UPDATE tbl_addr SET st_addr = '부산시' WHERE id = 1;
...
-- 그래서 나중에 입력한 데이터를 삭제하려고한다.
SELECT * FROM tbl_addr;

-- 성춘향 데이터들을 확인 해 봐야한다.
SELECT * FROM tbl_addr WHERE st_name = '성춘향';

-- 성춘향의 2번째 데이터인 id가 4인 데이터를 삭제
DELETE FROM tbl_addr WHERE id = 4;

SELECT * FROM tbl_addr;

-- DataBase의 Table 생성할 때 
